...
while True:
    ...
    if ret == True:
        ...
        circles = cv2.HoughCircles(threshold, cv2.HOUGH_GRADIENT, 1, 100, param1=40, param2=15, minRadius=20, maxRadius=40)
        if circles is not None:
            ...
            print(test)
            detected_circles = np.reshape(detected_circles,(np.shape(detected_circles)[1],np.shape(detected_circles)[2]))
